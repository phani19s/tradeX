...
function SupportChatModal({ isOpen, onClose }) {
  ...
  async function fetchMessages() {
    try {
      const response = await api.get("/support/chat", getAuthHeaders());
  ...
  async function handleSendMessage(e) {
    ...
    setLoading(true);
    try {
      await api.post("/support/chat", { message: newMessage }, getAuthHeaders());
      setNewMessage("");
      fetchMessages();
    } catch (error) {
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isUser ? "items-end" : "items-start"}`}>
                <div 
                  className={`max-w-[80%] rounded-2xl p-3 text-sm shadow-sm ${isUser ? "text-white" : ""}`}
                  style={{ 
